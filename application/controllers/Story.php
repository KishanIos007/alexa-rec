...
class Story extends BaseController
{
    ...
    public function index()
    {
        ...
        $this->global['pageTitle'] = 'Story';
        $this->load->library('pagination');
        $count = $this->story_model->getAllCount($params);
        $returns = $this->paginationCompress ( "story/", $count, 1 );
		if(!empty($params['searchText'])) {
			$audios = $this->story_model->getAllUserInfo($params, $returns["page"], 0);
		}
		else
		{
        $audios = $this->story_model->getAll($params, $returns["page"], 0);
		}
        $data['audios'] = $audios;
        ...
        $this->loadViews("story", $this->global, $data , NULL);
    }
